...
export const nodesReducer = (
  ...
  const nodes = (nodesState && nodesState.length && currentStateIndex >= 0 &&
  ...
  switch (action.type) {
    case 'ADD_CONNECTION': {
      ...
      if ( inputIsNotConnected ) {
        const allowCircular = circularBehavior === 'warn' ||
    ...
    case 'DESTROY_TRANSPUT': {
      ...
      return connections.reduce((nodes, cnx) => {
        const [input, output] = transputType === 'input' ? [transput, cnx]
          : [cnx, transput];
        const id = output.nodeId + output.portName + input.nodeId +
                   input.portName;
        delete cache.current.connections[id];
        deleteConnection({ id });
        return removeConnection(nodes, input, output);
