...
Whoa there buster, no one's twistin' your arm. In seriousness though, there are several great node editors for Javascript out there, and Flume is inspired by some of the best features of many of them. However, where many of these libraries focus primarily on providing a node editor for anything and everything, Flume is focused on providing a node editor primarily for modeling application/business logic, **and** providing an engine for _running_ that logic. It's also specifically designed to be as user-friendly as possible for end-users, not just developers.
